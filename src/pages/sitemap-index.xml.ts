import { getGuides, listings, categories, regions } from '../lib/data';
...
export const GET = () => {
  ...
  const listingDetailUrls = listings.flatMap((listing) =>
    listing.categorySlugs.map((categorySlug) => `/listings/${categorySlug}/${listing.citySlug}/${listing.slug}`)
  );
  const guideUrls = getGuides().map((guide) => `/guides/${guide.slug}`);
